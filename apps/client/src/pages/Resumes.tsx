...
function CreateActions({
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (error || !data) {
      ...
      onExtractDone?.({ id: data.id, error: null });
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/resumes/analyze?id=${r.id}`); }} title="Analyze" className="p-1.5 rounded-lg text-(--color-text-tertiary) hover:text-(--color-accent) hover:bg-(--color-accent-subtle) transition-all duration-200 cursor-pointer">
                    ...
                    <button onClick={(e) => handleDelete(e, r.id, r.title)} title="Delete" className="p-1.5 rounded-lg text-(--color-text-tertiary) hover:text-(--color-error) hover:bg-(--color-error-bg) transition-all duration-200 cursor-pointer">
